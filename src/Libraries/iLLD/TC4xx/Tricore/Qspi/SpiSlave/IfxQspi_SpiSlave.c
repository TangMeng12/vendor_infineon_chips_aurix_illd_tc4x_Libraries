...
void IfxQspi_SpiSlave_initModule(IfxQspi_SpiSlave *handle, const IfxQspi_SpiSlave_Config *config)
{
    ...
    /* Protocol Configuration */
    {
        ...
        chConfig.mode.parityMode      = Ifx_ParityMode_even;
    ...
    else
    {
        ...
        {
            IfxQspi_SpiSlave_initLvdsPin(config->pins_n);
        }
    }
    ...
    if (config->dma.useDma)
    {
        ...
        /* Dma channel interrupt configuration */
        {
            ...
            IfxSrc_init(src, config->isrProvider, config->txPriority, config->vmId); /* Must be configurable */
            IfxSrc_enable(src);

            src = IfxDma_getSrcPointer(dmaSFR, (IfxDma_ChannelId)config->dma.rxDmaChannelId);
            IfxSrc_init(src, config->isrProvider, config->rxPriority, config->vmId); /* Must be configurable */
            IfxSrc_enable(src);
        }
    }
