...
/** \brief Macro to calculate the offset for loading LOLUT image into DCCM memory
 */
#define IFXADC_CDSP_LOLUT_IMAGE_OFFSET           (0x40U)
...
typedef struct
{
    ...
    IfxAdc_CdspReinitialSrc         reinitialEventSrc;       /**< \brief brief Re-initialization event source */
 ...
 *
 * \code
 *      IfxAdc_CdspCore_0, IfxAdc_CdspCore_2, IfxAdc_CdspCore_3 DSP cores need to disable.
 *      update the vlaue in dspCoresDisableValue = (1u << IfxAdc_CdspCore_0) | (1u << IfxAdc_CdspCore_2) | (1u << IfxAdc_CdspCore_3)
 *      uint32 dspCoresDisableValue = 13u;
 *      IfxAdc_Cdsp_disableMultipleCores(dsp, dspCoresDisableValue)
 * \endcode
 *
 */
IFX_INLINE void IfxAdc_Cdsp_disableMultipleCores(IfxAdc_Cdsp *cdsp, uint32 dspCoresDisableValue);
...
IFX_INLINE uint16 IfxAdc_Cdsp_getNumOfSamples(IfxAdc_Cdsp_Dsp *dsp)
{
    volatile uint32 *fftSamples = (uint32 *)(IFXADC_CDSP_GETDCCM_ADDRESS(dsp->coreId) + IFXADC_CDSP_FFT_INPUT_SAMPLE_DCCM_OFFSET);
...
IFX_INLINE void IfxAdc_Cdsp_disableMultipleCores(IfxAdc_Cdsp *cdsp, uint32 dspCoresDisableValue)
{
    cdsp->cdspSFR->GLOBHCD.B.CHHLTD = dspCoresDisableValue;
}
