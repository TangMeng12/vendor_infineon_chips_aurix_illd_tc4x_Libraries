...
/** \brief De-initializes the PSI5S module, releasing all allocated resources and resetting the module to its initial state.
 * \param psi5s Pointer to the Psi5s instance to be de-initialized.
 * \return None
 */
 ...
 * \param psi5s Pointer to the PSI5S module instance to be initialized.
 ...
 *              - TRUE: Module initialized with valid configuration.
 *              - FALSE: Module initialization failed due to invalid configuration.
 *
...
/** \brief Initializes the PSI5S module configuration parameters.
 * \param config Pointer to the Psi5s_Config structure that will be initialized.
 * \param psi5s Pointer to the PSI5S module SFR handle.
 * \return None
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
 ...
 * \param psi5s Pointer to the Psi5s module instance to be configured.
 * \return None
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
...
/** \brief Enables or disables the trigger counters for the specified channels in the PSI5S module.
 * \param psi5s Pointer to the IfxPsi5s_Psi5s object. This is the handle to the PSI5S module instance.
 * \param channels Bitmask representing the channels for which the trigger counters should be modified.
 *                     A channel is selected if its corresponding bit is set in this parameter.
 * \param mask Bitmask that specifies whether the trigger counters should be modified or not.
 *                   If a bit is set in this mask, the corresponding channel's trigger counter should be modified.
 *                   If a bit is not set, the corresponding channel's trigger counter should not be modified.
 * \return None
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
...
/** \brief Enables or disables specific channels in the PSI5S module.
 * \param psi5s Pointer to the Psi5s module instance.
 * \param channels Bitmask representing the channels to be enabled or disabled.
 *                     - Bit = 0: Channel is disabled.
 *                     - Bit = 1: Channel is enabled.
 * \param mask Bitmask indicating which channels' enable/disable state should be updated.
 *                - Bit = 0: Channel state remains unchanged.
 *                - Bit = 1: Channel state is updated based on the corresponding bit in the channels parameter.
 * \return None
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
 ...
 * \param channel Pointer to the PSI5S channel structure to be initialized.
 * \param config Constant pointer to the PSI5S channel configuration structure.
 * \return Channel initialization status.
 *                     - TRUE: Channel initialized successfully
 *                     - FALSE: Channel initialization failed due to invalid configuration
 *                              or missing resource.
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
...
/** \brief Initializes the PSI5S channel configuration structure with default values.
 * \param config Pointer to the IfxPsi5s_Psi5s_ChannelConfig structure to be initialized.
 *              Must not be NULL.
 * \param psi5s Pointer to the IfxPsi5s_Psi5s instance to be used for hardware-specific
...
/** \brief Sends data through a specified PSI5S channel.
 * \param channel Pointer to the PSI5S channel structure to be used for transmission.
 * \param data The 32-bit data to be sent over the PSI5S channel.
 * \return boolean Data transfer status
 *                     - TRUE: Data transfer is successfully
 *                     - FALSE: Data transfer has failed.
 *
 * Usage Example: \ref IfxLld_Psi5s_Psi5s_Usage
 *
 */
...
/** \brief Enable the PSI5S module kernel
 * \param psi5s pointer to the base of PSI5S register handle
 * \return None
 */
IFX_EXTERN void IfxPsi5s_Psi5s_enableModule(Ifx_PSI5S *psi5s);

/** \brief Retrieves the configured baudrate for the PSI5S module.
 * \param psi5s Pointer to the PSI5S module's SFR handle.
 * \param ascConfig Pointer to the PSI5S's ASC configuration structure.
 * \return Returns the configured baudrate from the hardware.
 */
 ...
 * \param psi5s Pointer to the PSI5S module's SFR handle.
 *                  Must point to a valid PSI5S object; otherwise, undefined behavior occurs.
 * \return None
 */
...
/** \brief Gets the read frame status for the specified channel.
 * \param psi5s Pointer to the Psi5s module instance. Must not be NULL.
 * \param channelId The channel ID for which the read frame status is to be checked.
 * \return Availability of a read frame.
 *            - TRUE : A frame is available for reading in the specified channel.
 *            - FALSE: No frame is available to read.
 */
