...
/** \brief Clears the status of the specified flash module.
 * \param flash The identifier or address of the flash module to clear the status for.
 *               The parameter specifies which flash module's status is to be cleared.
 *               Range: 0x00000000 to 0xFFFFFFFF (valid flash module identifiers).
 * \return None
 */
...
/** \brief Erases multiple consecutive sectors in the flash memory starting from a specified address.
 * \param sectorAddr The starting address of the first sector to be erased. This must be a valid sector address as defined by the hardware.
 * \param numSector The number of consecutive sectors to erase. This must be a positive integer and within the range of available sectors in the flash memory.
 * \return None
 *
 * Usage Example:
 * \code
 ...
 * \param sectorAddr The starting address of the sector to erase. Must be aligned to the sector size.
 * \return None
 *
 * Usage Example:
 * \code
...
/** \brief Erases and verifies multiple flash memory sectors starting from a specified address.
 * \param sectorAddr The starting sector address to erase and verify.
 * \param numSector The number of sectors to erase and verify.
 * \return None
 */
...
/** \brief Loads a page from flash memory into RAM using 2X32-bit access.
 * \param pageAddr pageAddr start address of page which should be programmed
 * \param wordL Lower Address word
 * \param wordU Upper Address word
 * \return None
 ...
 * \param sectorAddr The starting address of the sector to suspend or resume operations on. Must be a valid flash sector address.
 * \param numSector The number of sectors to suspend or resume operations on. Must be a positive integer.
 * \return None
 */
...
/** \brief Counts the user content in the flash memory starting from the specified word address.
 * \param wordAddr The starting word address in the flash memory where the user content count begins.
 * \return None
 */
IFX_INLINE void IfxFlash_userContentCount(uint32 wordAddr);

/** \brief Checks the user margin count for a specific word address in flash memory.
 * \param wordAddr The 32-bit word address in flash memory for which the user margin count is to be checked.
 * \return None
 */
IFX_INLINE void IfxFlash_userMarginCount(uint32 wordAddr);

/** \brief Counts the user threshold voltage (Vth) for a specified word address in the flash memory.
 * \param wordAddr The 32-bit word address in the flash memory for which the Vth count is to be determined.
 * \return None
 */
IFX_INLINE void IfxFlash_userVthCount(uint32 wordAddr);

/** \brief Verifies if a specified page in flash memory has been erased.
 * \param pageAddr The address of the page to verify.
 * \return None
 *
 * Usage Example:
 *
...
/** \brief Reallocates or switches the active flash bank.
 ...
 * \param flashType The type of flash memory being accessed (e.g., program flash, data flash).
 * \return 0 on success, != 0 if invalid or not available page is selected
 *
 * Usage Example:
 * \code
...
/** \brief Wakes up a specified flash bank from low-power mode.
 * The command interface can only wakeup those banks which are allocated to the command channel.
 * \param bankselect The index or identifier of the flash bank to wake up.
 * \return None
 *
 * Usage Example:
 *  \code
...
/** \brief Retrieves the flash bank associated with the given memory address.
 * \param address The memory address for which to determine the flash bank.
...
/** \brief Retrieves the error status of the Flash command interface.
...
#if defined(IFXFLASH_PFLASH_P51_END)
    if (((pageAddr >= IFXFLASH_PFLASH_P00_START) && (pageAddr <= IFXFLASH_PFLASH_P51_END))) // program flash
#else
    if (((pageAddr >= IFXFLASH_PFLASH_P00_START) && (pageAddr <= IFXFLASH_PFLASH_P41_END))) // program flash
#endif	/* #if defined(IFXFLASH_PFLASH_P51_END) */
    {
        *addr1 = 0x50;
        return 0;
    }
...
#else
    if ((address >= IFXFLASH_PFLASH_P00_START) && (address <= IFXFLASH_PFLASH_P41_END)) /* Program Flash */
#endif/* #if defined(IFXFLASH_PFLASH_P51_END) */	
    {
