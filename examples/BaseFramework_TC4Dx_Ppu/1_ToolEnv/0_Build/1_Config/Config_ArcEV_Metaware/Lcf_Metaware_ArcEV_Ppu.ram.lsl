...
MEMORY
{
    ...
    LMU7.CACHED        : ORIGIN = 0x90780000,                LENGTH = 1024K         #LMU7 and LMU8 are reserved for PPU
    CSM                : ORIGIN = 0x92080000,                LENGTH = 256K          #Local memory for PPU. Used as cached address (Tricore shall not access this memory during runtime)
    ...
    LMU_NOT_ALLOCATED2 : ORIGIN = 0xB0800000,                LENGTH = 512K          #Define to set up MPU not allocated allocation is done by TriCore but PPU might read
...
_fMPU_REGION2 = ADDR(LMU7.CACHED);
_eMPU_REGION2 = ADDR(LMU7.CACHED) + 512k;                                           #LMU7 and LMU8 regions can't be hoined together in the same MPU because of address misalignment
_pMPU_REGION2 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_EXECUTE | _MPU_PERMISSION_WRITE);

_fMPU_REGION3 = ADDR(LMU7.CACHED) + 512k;
_eMPU_REGION3 = ADDR(LMU7.CACHED) + 1024k;                                          #LMU7 and LMU8 regions can't be hoined together in the same MPU because of address misalignment
_pMPU_REGION3 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_EXECUTE | _MPU_PERMISSION_WRITE);

_fMPU_REGION4 = ADDR(CSM);
_eMPU_REGION4 = ADDR(CSM) + SIZEOF(CSM);
_pMPU_REGION4 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_EXECUTE | _MPU_PERMISSION_WRITE);

_fMPU_REGION5 = ADDR(VCCM);
_eMPU_REGION5 = ADDR(VCCM) + SIZEOF(VCCM);
_pMPU_REGION5 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_WRITE);

_fMPU_REGION6 = ADDR(LMU_NOT_ALLOCATED1);
_eMPU_REGION6 = ADDR(LMU_NOT_ALLOCATED1) + SIZEOF(LMU_NOT_ALLOCATED1);
_pMPU_REGION6 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_WRITE);

_fMPU_REGION7 = ADDR(LMU_NOT_ALLOCATED2);
_eMPU_REGION7 = ADDR(LMU_NOT_ALLOCATED1) + SIZEOF(LMU_NOT_ALLOCATED1);
_pMPU_REGION7 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_WRITE);

_fMPU_REGION8 = ADDR(PER_SPACE_NO_ALOC1);
_eMPU_REGION8 = ADDR(PER_SPACE_NO_ALOC1) + SIZEOF(PER_SPACE_NO_ALOC1);
_pMPU_REGION8 = (_MPU_PERMISSION_READ | _MPU_PERMISSION_WRITE);
...
SECTIONS 
{
    ...
    GROUP ALIGN(LCF_DCACHE_LINESIZE) BLOCK(8):
    {
        .csm.data: {* (.csm.data) }
        .padding ALIGN(LCF_DCACHE_LINESIZE) SIZE(2): {} = FILL(0x781E)
    } = FILL(0x781E) > CSM AT>LMU7.CACHED
    ...
    GROUP ALIGN(LCF_DCACHE_LINESIZE) BLOCK(8):
    {
        ...
        .vecmem_data : {}
        .padding ALIGN(LCF_DCACHE_LINESIZE) SIZE(2): {} = FILL(0x781E)
    } = FILL(0x781E) > VCCM AT > LMU7.CACHED
