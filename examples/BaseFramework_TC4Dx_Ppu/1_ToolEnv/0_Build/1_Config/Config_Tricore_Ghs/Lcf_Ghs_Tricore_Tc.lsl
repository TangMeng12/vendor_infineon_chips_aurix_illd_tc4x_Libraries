...
SECTIONS
{
    ...
    /* PFLASH Sections */
    /* Fixed memory Allocations for _START for CPU 0*/
...
/* Fixed memory Allocations for _START for CPU 1 to 5*/
	...
	.CPU2_text            ALIGN(2) : { "Ifx_Ssw_Tc2.*(.text)" "Ifx_Ssw_Tc2.*(.text.*)" "Cpu2_Main.*(.text)" "Cpu2_Main.*(.text.*)" "*(.text_cpu2)" "*(.text_cpu2)" } > pfls2
	.CPU1_text            ALIGN(2) : { "Ifx_Ssw_Tc1.*(.text)" "Ifx_Ssw_Tc1.*(.text.*)" "Cpu1_Main.*(.text)" "Cpu1_Main.*(.text.*)" "*(.text_cpu1)" "*(.text_cpu1)" } > pfls1
	.CPU0_text            ALIGN(2) : { "Ifx_Ssw_Tc0.*(.text)" "Ifx_Ssw_Tc0.*(.text.*)" "Cpu0_Main.*(.text)" "Cpu0_Main.*(.text.*)" "*(.text_cpu0)" "*(.text_cpu0)" } > pfls0
