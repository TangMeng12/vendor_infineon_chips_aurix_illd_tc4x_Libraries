...
/*Sections located at absolute fixed address*/
    ...
    SECTIONS
    {
        .start_tc0 (LCF_STARTPTR_NC_CPU0) : FLAGS(rxl) { KEEP (*(.start)); } > cpu0_dlmu_nc
    ...
    SECTIONS
    {
        .start_tc1 (LCF_STARTPTR_NC_CPU1) : FLAGS(rxl) { KEEP (*(.start_cpu1)); } > cpu1_dlmu_nc
        .start_tc2 (LCF_STARTPTR_NC_CPU2) : FLAGS(rxl) { KEEP (*(.start_cpu2)); } > cpu2_dlmu_nc
        .start_tc3 (LCF_STARTPTR_NC_CPU3) : FLAGS(rxl) { KEEP (*(.start_cpu3)); } > cpu3_dlmu_nc
        .start_tc4 (LCF_STARTPTR_NC_CPU4) : FLAGS(rxl) { KEEP (*(.start_cpu4)); } > cpu4_dlmu_nc
        .start_tc5 (LCF_STARTPTR_NC_CPU5) : FLAGS(rxl) { KEEP (*(.start_cpu5)); } > cpu5_dlmu_nc
...
SECTIONS
{
    ...
    .ppu_lmuram (ORIGIN(lmuram_nc) + LENGTH(lmuram_nc) - LCF_PPU_LMURAM_SIZE) (NOLOAD) : FLAGS(aw)
    {
    . = ALIGN(4);
    __PPU_LMURAM_START = .;
    . += LCF_PPU_LMURAM_SIZE;
...
SECTIONS
{
    ...
    CORE_SEC(.ppu_code) (ORIGIN(lmuram) + LENGTH(lmuram) - LCF_PPU_LMURAM_SIZE - LCF_PPU_FLASH_SIZE) (NOLOAD) : FLAGS(axl)
    {
    __PPU_IF_CONSTANTS = .;
    __PPU_CODE_START = .;
    . += LCF_PPU_FLASH_SIZE;
